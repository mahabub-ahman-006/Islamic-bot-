...
console.log(`${crayon(``)}${sky(`NAZRUL DATA`)} - bot name : ${chalk.blueBright((!global.config.BOTNAME) ? "Nazrul" : global.config.BOTNAME)} \n${sky(`NAZRUL DATA `)} - bot id : ${chalk.blueBright(api.getCurrentUserID())} \n${sky(`NAZRUL DATA`)} - bot prefix : ${chalk.blueBright(global.config.PREFIX)}\n${sky(`NAZRUL DATA`)} - deployed ${chalk.blueBright(operator)} bot operators and ${chalk.blueBright(admin)} admins`);
if (global.config.approval) {
  console.log(`${sky(`NAZRUL DATA`)} - deployed ${chalk.blueBright(approved)} approved groups`)
} 
if (global.config.premium) {
  console.log(`${sky(`NAZRUL DATA `)} - deployed ${chalk.blueBright(premium)} premium users`)
}
